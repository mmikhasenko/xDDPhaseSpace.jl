function σ2of3_pm(σ3, msq, s)
    regularpart = (msq[1] + msq[3] + (σ3 + msq[1] - msq[2]) * (s - σ3 - msq[3]) / (2 * σ3))
    # 
    sqrt_σ3 = sqrt(σ3)
    ms = sqrt.(msq)
    sqrt_s = sqrt(s)
...
function σ3of1_pm(σ1, msq, s)
    regularpart = (msq[1] + msq[2] + (σ1 + msq[2] - msq[3]) * (s - σ1 - msq[1]) / (2 * σ1))
    # 
    sqrt_σ1 = sqrt(σ1)
    ms = sqrt.(msq)
    sqrt_s = sqrt(s)
...
function σ3of2_pm(σ2, msq, s)
    regularpart = (msq[1] + msq[2] + (σ2 + msq[1] - msq[3]) * (s - σ2 - msq[2]) / (2 * σ2))
    # 
    sqrt_σ2 = sqrt(σ2)
    ms = sqrt.(msq)
    sqrt_s = sqrt(s)
...
# 
σ3of1(σ1, cos23, msq, s) =
    msq[1] +
    msq[2] +
    (s - σ1 - msq[1]) * (σ1 + msq[2] - msq[3]) / (2σ1) +
    cos23 * sqrt(λ(s, σ1, msq[1]) * λ(σ1, msq[2], msq[3])) / (2σ1)
# 
σ2of1(σ1, cos23, msq, s) =
    msq[1] + msq[3] + (s - σ1 - msq[1]) * (σ1 - msq[2] + msq[3]) / (2σ1) -
    cos23 * sqrt(λ(s, σ1, msq[1]) * λ(σ1, msq[2], msq[3])) / (2σ1)
# 
...
function mapdalitz(method::HookSqrtDalitzMapping{3}, x, ms, s)
    ...
    sqrt_σ3 =
        x[1] < 0.5 ? sqrt_σ3_0 + (x[1] / 0.5) * (sqrt_σ3_i - sqrt_σ3_0) : # straight path
        sqrt_σ3_i + ((x[1] - 0.5) / 0.5) * (sqrt_σ3_e - sqrt_σ3_i) # straight path
    #
    jacobian = x[1] < 0.5 ? (sqrt_σ3_i - sqrt_σ3_0) / 0.5 : (sqrt_σ3_e - sqrt_σ3_i) / 0.5
    # 
...
function mapdalitz(method::HookSqrtDalitzMapping{2}, x, ms, s)
    ...
    sqrt_σ2 =
        x[1] < 0.5 ? sqrt_σ2_0 + (x[1] / 0.5) * (sqrt_σ2_i - sqrt_σ2_0) : # straight path
        sqrt_σ2_i + ((x[1] - 0.5) / 0.5) * (sqrt_σ2_e - sqrt_σ2_i) # straight path
    #
    jacobian = x[1] < 0.5 ? (sqrt_σ2_i - sqrt_σ2_0) / 0.5 : (sqrt_σ2_e - sqrt_σ2_i) / 0.5
    # 
