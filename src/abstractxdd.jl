...
function ρ_thr(d::AbstractxDD, m::Complex)
    integrand(x, f) = f[1:2] .= reim(integrand_mapped_thr(d, m^2, x))
    v = cuhre(integrand, 2, 2)[1]
    complex(v...) / (8π)^2
end

function ρ_thr(d::AbstractxDD, m::Real)
    integrand(x, f) = f[1] = real(integrand_mapped_thr(d, m^2, x))
